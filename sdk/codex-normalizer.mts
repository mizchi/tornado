...
function getItemId(item: CodexItem): string {
  return typeof item?.id === "string" ? item.id : "";
}

export function getCodexItemId(item: CodexItem): string {
  return getItemId(item);
}

function toolNameForEvent(item: CodexItem): string {
  return String(item.server || item.server_name || item.tool || "mcp");
}

function formatMcpLabel(item: CodexItem): string {
...
function formatMcpArgs(args: UnknownRecord | undefined): string {
  if (!args) return "";
  const subagent = collapseWhitespace(args.subagent_type || args.agent_type || "");
...
function formatTodoSummary(items: TodoItem[] = []): string {
  ...
  const next = collapseWhitespace(items.find((item) => !item.completed)?.text || "");
...
function commandFallback(item: CodexItem): string {
  return typeof item.exit_code === "number"
    ? `exit_code=${item.exit_code}`
    : "";
}

function formatCommandLabel(item: CodexItem): string {
  return `Bash(${truncate(collapseWhitespace(item.command || ""), 80)})`;
}

function extractMcpResultText(item: CodexItem): string {
...
export function formatItemStartLog(item: CodexItem): string | null {
  switch (item?.type) {
    ...
    case "mcp_tool_call": {
      ...
      if (toolName === "Task" && argsSummary) return `Task(${truncate(argsSummary, 100)})`;
...
export function formatItemUpdateLog(item: CodexItem): string | null {
  switch (item?.type) {
    case "command_execution": {
...
export function formatItemCompleteLog(item: CodexItem): string | null {
  switch (item?.type) {
    case "command_execution": {
      const line = lastNonEmptyLine(commandOutput(item));
    ...
    case "mcp_tool_call": {
      ...
      if (output) return `MCP(${truncate(formatMcpLabel(item), 80)}) => ${truncate(output, 100)}`;
  ...
  switch (item?.type) {
    case "command_execution": {
      ...
      return {
        ...
        tool_name: "Bash",
        content: output,
        _display: formatItemCompleteLog(item) || `Bash: exit=${item.exit_code || 0}`,
      };
    }
    case "file_change": {
      const changes = item.changes || [];
      ...
      return {
        ...
        _display: formatItemCompleteLog(item) || `Edit: ${truncate(summary, 80)}`,
      };
    }
    case "mcp_tool_call": {
